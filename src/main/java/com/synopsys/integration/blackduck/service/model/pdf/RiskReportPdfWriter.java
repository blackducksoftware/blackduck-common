...
public class RiskReportPdfWriter {
    ...
    private PDRectangle writeDate(float startingHeight, LocalDateTime date) throws IOException {
        String formattedDateString = String.format("Created on %s.", date.format(DateTimeFormatter.ofPattern("MM/dd/yyyy - HH:mm:ss")).replace("-", "at"));
        return pdfManager.writeText(5, startingHeight - 10, formattedDateString, PDType1Font.TIMES_ITALIC, 8, Color.BLACK);
    }
